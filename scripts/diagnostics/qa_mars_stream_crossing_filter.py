...
def stratified_sample_removed_pairs(
    ...
    """Stratified by per-network pair count; prefer one pair per network.

    Returns a DataFrame with the sampled rows plus a 'size_bucket' column
    and 'drop_rate' column (per-network).
    """
    df_removed = df_all[df_all["stream_crossing_drop"]].copy()

    # Per-network stats over ALL pairs (kept + dropped)
    ...
    for label, lo, hi, n_target in size_buckets:
        ...
        # Spread across drop_rate within the bucket: sort by drop_rate,
        # pick n_target networks at evenly spaced ranks.
        if not bucket_nets:
            log.warning("Bucket %s: no networks available", label)
            continue
        bucket_sorted = (
            per_net.loc[bucket_nets].sort_values("drop_rate").index.tolist()
        )
        if len(bucket_sorted) <= n_target:
            chosen_nets = bucket_sorted
        else:
            # Even rank-spaced indices then jitter with the rng to avoid
            # always picking the same networks if reproducibility were a
            # concern; the seed is fixed, so this is deterministic.
    ...
    # If we still need more samples, top up from any remaining networks
    if len(picks) < n_samples:
        remaining = df_removed[
            ~df_removed["network_id"].isin(used_networks)
        ]
...
# ---------------------------------------------------------------------------
# Crossed-segment detection (mirrors Phase 3A filter logic)
# ---------------------------------------------------------------------------
# detect_crossed_segments + plot_removed_pair + the legend handles live in
# channel_heads.pairing / channel_heads.viz (shared with notebooks/diagnostics/).
