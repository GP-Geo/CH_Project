...
for R in regA regB regC; do
  ...
  step "$R:patches"        "$LOG_DIR/${R}_1_patches.log"  -- python scripts/build_cnn_patches_regime.py --regime "$R" -v
  step "$R:train_cnn"      "$LOG_DIR/${R}_2_cnn.log"      -- python scripts/train_cnn_regime.py --regime "$R" -v
  step "$R:combined_xgb"   "$LOG_DIR/${R}_3_xgb.log"      -- python scripts/train_combined_xgb_regime.py --regime "$R" -v
  step "$R:mars_combined"  "$LOG_DIR/${R}_4_mars.log"     -- python scripts/run_mars_combined_regime.py --regime "$R" -v
done
