"""Generic/Earth CNN embedding extraction and XGBoost-feature integration.

Canonical home of the generic CNN embedding helpers used by the Earth workflow
and by the Mars Phase-5 orchestration in :mod:`channel_heads.models.embeddings`:

1. :func:`extract_embeddings` — load a trained CNN and extract embeddings for
   all pairs in a manifest DataFrame.
2. :func:`merge_cnn_features` — merge CNN embedding columns into an existing
   feature DataFrame.

This implementation uses a **lenient** ``load_state_dict`` (no ``strict=`` flag,
no missing/unexpected-key check) and returns a manifest-keyed DataFrame. That is
deliberately distinct from the *strict* embedding/logit extractors in
:mod:`channel_heads.inference.regime` and :mod:`channel_heads.models.mars_combined`
— do not unify them. The historical import location
:mod:`channel_heads.cnn_features` re-exports these symbols as a compatibility
shim. Requires PyTorch.
"""
